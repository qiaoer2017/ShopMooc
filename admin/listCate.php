...
$sql = "SELECT `id`,`cName` FROM `mooc_cate` ORDER BY `id` LIMIT {$offset},{$pageSize}";
        ...
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td align="center"><input type="button" value="修改" class="btn"
                                          onclick="editCate(<?php echo $row['id']; ?>)">
                    <input type="button" value="删除" class="btn" onclick="delCate(<?= $row['id'] ?>)"/>
                </td>
            </tr>
        <?php endforeach; ?>
...
<script>
    ...
    function delCate(id) {
        if (window.confirm("确定要删除吗？删除之后不能恢复！",)) {
